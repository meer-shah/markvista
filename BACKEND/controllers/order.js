...
let recvWindow = 5000000;  // Maximum window for receiving the response (in ms)
 ...
 */
async function http_reques(endpoint, method, data, Info) {
...
async function http_request(endpoint, method, data, Info) {
    ...
    // Loop until we either succeed or exhaust the maximum retry attempts
    while (attempts < MAX_RETRIES && !success) {
        try {
            // Call the http_request function
            response = await http_reques(endpoint, method, data, Info);
            success = true;  // If no error, set success to true
        } catch (error) {
            attempts++;

            // If we failed and reached max attempts, break the loop
            if (attempts >= MAX_RETRIES) {
                console.error("Max retries reached. Could not complete the request.");
                break;
            }

            // Update recvWindow on each retry
            recvWindow += 5000;  // Increase recvWindow by 5000ms (5 seconds)
            console.log(`Retrying... Attempt ${attempts} with updated recvWindow: ${recvWindow}ms`);

            // Delay before retrying
            await new Promise(resolve => setTimeout(resolve, RETRY_DELAY));  // 3 seconds delay
        }
    }

    // Return the response or error after all attempts
    if (success) {
        return response;
    } else {
